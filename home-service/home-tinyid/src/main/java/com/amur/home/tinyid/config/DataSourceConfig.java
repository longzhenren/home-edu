package com.amur.home.tinyid.config;
...
public class DataSourceConfig {
    ...
    public DataSource getDynamicDataSource() {
        ...
        String names = binder.bind("datasource.tinyid.names", String.class).get();
        String dataSourceType = binder.bind("datasource.tinyid.type", String.class).get();
        ...
        for (String name : names.split(SEP)) {
            Map<String, Object> dsMap = binder.bind("datasource.tinyid." + name, Map.class).get();
    ...
    private DataSource buildDataSource(String dataSourceType, Map<String, Object> dsMap) {
        try {
            ...
            return DataSourceBuilder.create().driverClassName(driverClassName).url(url).username(username).password(password).type(type).build();
