...
public class DbSegmentIdServiceImpl implements SegmentIdService {
    ...
    public SegmentId getNextSegmentId(String bizType) {
        ...
        for (int i = 0; i < IdConstants.RETRY.getValue(); i++) {
    ...
    public SegmentId convert(TinyIdInfo idInfo) {
        ...
        // 默认20%加载
        segmentId.setLoadingId(segmentId.getCurrentId().get() + (long) idInfo.getStep() * IdConstants.LOADING_PERCENT.getValue() / 100);
        return segmentId;
    }
}
